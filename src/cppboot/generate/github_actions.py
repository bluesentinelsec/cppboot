...
from cppboot.generate.web import EMRUN_BROWSER, EMRUN_TIMEOUT_SECONDS
...
def _github_actions_ios_workflow(ctx: _Context) -> str:
    """iOS XCFramework: build device+simulator slices, verify, Simulator tests."""
    return f"""\
# Generated by cppboot (--with-ios-ci)
# iOS XCFramework: build static device (arm64) and simulator (arm64/x86_64)
# libraries, package and verify the XCFramework, and run the consumer package
# tests in an iOS Simulator.
name: iOS
...
jobs:
  ios:
    name: iOS (XCFramework / simulator)
    ...
    steps:
      ...
      - name: Build Debug XCFramework and consumer apps
        run: |
          bash scripts/build_ios_xcframework.sh Debug build/ios/debug
          bash scripts/build_ios_test_apps.sh \\
            build/ios/debug/{ctx.name}.xcframework Debug build/ios/consumer-debug

      - name: Build Release XCFramework and consumer apps
        run: |
          bash scripts/build_ios_xcframework.sh Release build/ios/release
          bash scripts/build_ios_test_apps.sh \\
            build/ios/release/{ctx.name}.xcframework Release build/ios/consumer-release

      - name: Run Release package tests in iOS Simulator
        env:
          BUILD_DIR: build/ios/consumer-release/iphonesimulator
        run: bash scripts/run_ios_tests.sh

      - name: Stage iOS package metadata
        id: ios_package
        run: |
          version="$(tr -d '[:space:]' < VERSION | sed 's/^v//;s/^V//;s/#.*//')"
          echo "artifact_name={ctx.name}-ios-xcframework-release-${{version}}" >> "${{GITHUB_OUTPUT}}"
          echo "artifact_path=build/ios/release/{ctx.name}-ios-xcframework-release-${{version}}.zip" >> "${{GITHUB_OUTPUT}}"

      - name: Upload iOS package
        uses: actions/upload-artifact@v7
        with:
          name: ${{{{ steps.ios_package.outputs.artifact_name }}}}
          path: ${{{{ steps.ios_package.outputs.artifact_path }}}}
          if-no-files-found: error
          retention-days: 14
"""


def _release_ios_job(ctx: _Context) -> str:
    """`build-ios` job fragment for the generated release workflow."""
    return f"""
  build-ios:
    ...
    steps:
      ...
      - name: Verify Release XCFramework version and slices
        run: >
          bash scripts/verify_ios_xcframework.sh
          build/ios/release/{ctx.name}.xcframework
          "${{RELEASE_VERSION}}"
      ...
      - name: Upload iOS Release XCFramework
        uses: actions/upload-artifact@v7
        with:
          name: {ctx.name}-ios-xcframework-release-${{{{ needs.prepare.outputs.version }}}}
          path: build/ios/release/{ctx.name}-ios-xcframework-release-${{{{ needs.prepare.outputs.version }}}}.zip
          if-no-files-found: error
          retention-days: 14
"""


def _web_package_step(ctx: _Context, version_expr: str) -> str:
    """Shared package step body: install the wasm library + bundle the demo."""
    return f"""\
          set -euo pipefail
          stem="{ctx.name}-web-wasm32-release-{version_expr}"
          ...
          mkdir -p "${{prefix}}/demo"
          cp build/web-release/bin/{ctx.name}.html build/web-release/bin/{ctx.name}.js \\
             build/web-release/bin/{ctx.name}.wasm "${{prefix}}/demo/"
          (cd dist && zip -r "${{stem}}.zip" "${{stem}}" >/dev/null)
          echo "artifact_name=${{stem}}" >> "${{GITHUB_OUTPUT}}"
          echo "artifact_path=dist/${{stem}}.zip" >> "${{GITHUB_OUTPUT}}\""""


def _github_actions_web_workflow(ctx: _Context) -> str:
    """Web/Emscripten: wasm library + canvas demo, browser tests via emrun."""
    return f"""\
# Generated by cppboot (--with-web-ci)
# Web/Emscripten: build the wasm32 static library, the HTML5 canvas demo, and
# the browser test page; run the tests in headless Chrome via emrun; package
# the installed library with the demo bundled.
name: Web
    ...
    steps:
      ...
      - name: Configure Debug
        run: >
          ...
          -DCMAKE_BUILD_TYPE=Debug
          -D{ctx.macro}_BUILD_TESTS=ON

      - name: Build Debug
        run: cmake --build build/web-debug --parallel

      - name: Test Debug in headless Chrome
        run: >
          emrun --browser={EMRUN_BROWSER}
          --browser_args="--headless=new --no-sandbox --disable-gpu"
          --kill_exit --timeout {EMRUN_TIMEOUT_SECONDS}
          build/web-debug/bin/{ctx.target}_web_test.html
      ...
      - name: Build Release
        run: cmake --build build/web-release --parallel

      - name: Test Release in headless Chrome
        run: >
          emrun --browser={EMRUN_BROWSER}
          --browser_args="--headless=new --no-sandbox --disable-gpu"
          --kill_exit --timeout {EMRUN_TIMEOUT_SECONDS}
          build/web-release/bin/{ctx.target}_web_test.html

      - name: Verify canvas demo build
        run: |
          set -euo pipefail
      ...
      - name: Package web library and demo
        id: package
        run: |
{_web_package_step(ctx, '$(tr -d "[:space:]" < VERSION | sed "s/^v//;s/^V//;s/#.*//")')}
...
def _release_web_job(ctx: _Context) -> str:
    """`build-web` job fragment for the generated release workflow."""
    return f"""
  build-web:
    name: build-web (Emscripten/browser)
    needs: prepare
    runs-on: ubuntu-latest
    timeout-minutes: 35
    env:
    ...
    steps:
      ...
      - name: Package web library and demo
        id: pkg
        run: |
{_web_package_step(ctx, "${RELEASE_VERSION}")}

      - name: Upload web Release zip
        uses: actions/upload-artifact@v7
        with:
...
"""
    if ctx.with_ios_ci:
        needs.append("build-ios")
        ios_job = _release_ios_job(ctx)
    if ctx.with_web_ci:
        needs.append("build-web")
        web_job = _release_web_job(ctx)
    publish_needs = "[" + ", ".join(needs) + "]"
    return f"""\
# Generated by cppboot (default; --no-github-actions to skip)
