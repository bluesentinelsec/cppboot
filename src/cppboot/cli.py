...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Add a web/Emscripten package: HTML5 canvas game-loop demo, "
            "browser tests via headless Chrome, and GitHub Actions web.yml + "
            "release job (default: off; not compatible with --with-modules)."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"  web-ci: {'yes' if options.with_web_ci else 'no'}")
